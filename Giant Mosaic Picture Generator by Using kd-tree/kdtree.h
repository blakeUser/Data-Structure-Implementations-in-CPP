...
class KDTree
{
  ...
  public:
    ...
    // functions used for grading:
     typename KDTree<Dim>::KDTreeNode* helpToConstruct(vector<Point<Dim>>& newPoints, int dimension, int start, int end);
     int partition(vector<Point<Dim>>& newPoints, int left, int right, int pivotIndex, int dimension);
     Point<Dim> select(vector<Point<Dim>>& newPoints, int left, int right, int medianIndex, int dimension);
     void findHelper(int dimension, Point<Dim> & toReturn, Point<Dim> query, KDTreeNode * root) const;
